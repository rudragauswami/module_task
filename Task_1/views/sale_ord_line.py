...
class SaleOrderLine(models.Model):
    ...
    allowed_shutter_product_ids = fields.Many2many('product.product',relation='rel_line_allowed_shutter_products',
    ...
    def _compute_allowed_shutter_products(self):
        for line in self:
            ...
            if range_rule:
                valid_configs = self.env['product.product'].search([
                        ('range_config_id', '=', range_rule.id)
                ])

                line.allowed_shutter_product_ids = valid_configs.mapped('product_id')
